...
<Callout title="注意warning" type='error'>
...
<Callout title="Use Git from Git Bash only"> 
...
<Callout title="Git from the command line and also from 3rd-party software">
...
(推荐) 此选项将添加一些最小的 Git 包装程序,以避免将环境变量中的可选的 Unix 工具混淆。

您将能够从 Git Bash、命令提示符和 Windows PowerShell 以及任何在 PATH 中查找 Git 的第三方软件中使用 Git。
</Callout>

<Callout title="Use Git and optional Unix tools from the Command Prompt">
...
<Callout title="Use bundled OpenSSH">
...
<Callout title="Use external OpenSSH">
